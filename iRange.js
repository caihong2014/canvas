/*
 * method: used in wap/web range , even html5 input has already put the type range.
 * @param {form} {int} 
 * @param {to} {int}
 * @param {step} {int} step
 * @param {scale} 
 * @param {format}
 * @param {width}
 * @param {showLabels} default true
 * @param {showScale} default true
 *
 * time:2015-10-20
 * author: nan
 * contact: liyananhappy@hotmail.com
 */
;(function($, window, document, undefined) {
	...
	iRange.prototype = {
		...
		showPointerValue: function(pointer, position, animate) {
			...
			if(value.toString().length>8){
				fixFloatValue=parseFloat(value.toString().substr(0,5));
			}else{
				fixFloatValue=value;
			}
			console.log(fixFloatValue);
			if ($.isFunction(this.options.format)) {
				var type = this.isSingle() ? undefined : (pointer.hasClass('low') ? 'low' : 'high');
				text = this.options.format(fixFloatValue, type);
			} else {
				text = this.options.format.replace('%f', fixFloatValue);
			...
			this.setInputValue(pointer, fixFloatValue);
		},
	...
	$.fn[pluginName] = function(option) {
		...
		this.each(function() {
			...
			if (!data) {
				$this.data('plugin_' + pluginName, (data = new iRange(this, options)));
